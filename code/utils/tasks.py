...
def perform_task(command, login, password, all_users):
    user = authenticate(login, password, all_users)
    if not user:
        print("Invalid Login")
        return
    ...
    if command == 'print-all-accounts':
        print_all_accounts(all_users)
    elif command == 'print-oldest-account':
        print_oldest_account(all_users)
    elif command == 'group-by-age':
        print_group_by_age(all_users)
    elif command == 'print-children':
        print_children(user, all_users)
    elif command == 'find-similar-children-by-age':
        print_similiar_children_by_age(user, all_users)



def print_all_accounts(all_users):
    count_valid_accounts = len(all_users)
    print(count_valid_accounts)


def print_oldest_account(all_users):
...
def print_group_by_age(all_users):
    children_by_age = {}
    for user in all_users:
